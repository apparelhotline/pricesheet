...
- **The live-sync append builds a hand-written JSON body, so any `\` or `"` in an order's text (personalization, name, address) corrupts the payload → Google returns `400 INVALID_ARGUMENT - Invalid JSON payload` and that order is silently dropped.** Fix in scenario 4749764: a helper GET module reads `RECON!P1:P2` (`=CHAR(92)` backslash, `=CHAR(34)` quote — leave these cells in place), and every free-text field is wrapped `replace(replace(<expr>; <P1>; "_"); <P2>; "_")` before going into the body. Make has **no `toJSON()`** function and IML string literals can't reliably hold `\`/`"`, which is why the offending characters are sourced from cells instead of written inline.
- Make validates module references by **id order** — a module referenced in a `{{...}}` must have a lower `id` than the module using it, regardless of flow-array position (helper read is id 2, append is id 5). Off-by-one here surfaces only as a generic "Scenario validation failed - N problem(s)" at run time.
